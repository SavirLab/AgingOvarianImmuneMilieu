function sp=spider_plot(P, P_labels, axes_interval, axes_precision, varargin)
...
for ii = 1:num_of_points
    ...
    max_values(ii) = 60;% max(group_points);
    min_values(ii) = 0;%min(group_points);
...
for ii = 1:row_of_points
    ...
    % Plot data points
   sp(ii)= plot(x_circular, y_circular,...
        'Color', colors(ii, :),...
...
for hh = 1:num_of_points
    ...
    for ii = 2:length(radius)
        ...
        text(x_isocurves(ii, hh), y_isocurves(ii, hh), sprintf(sprintf('%%i', axes_precision), row_axis_labels(ii)),...
            'Units', 'Data',...
            'Color', 'k',...
