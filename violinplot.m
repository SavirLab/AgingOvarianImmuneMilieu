...
% This code is released under the terms of the BSD 3-clause license
    ...
    if isa(data, 'dataset') || isstruct(data) || istable(data)
        ...
        for n=1:length(catnames)
            ...
            violins(n) = Violin_age(thisData, n, varargin{:});
        end
        set(gca, 'xtick', 1:length(catnames), 'xticklabels', catnames,'FontSize',10)%,'FontWeight','Bold');
    ...
    elseif not(hascategories) && isvector(data)
        violins = Violin_age(data, 1, varargin{:});
        set(gca, 'xtick', 1);
    ...
    elseif ismatrix(data)
        ...
        if hascategories && length(cats) == size(data, 2)
            set(gca, 'xticklabels', cats,'FontSize',10)%,'FontWeight','Bold');
        end
